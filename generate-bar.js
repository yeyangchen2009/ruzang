 ...
 * Docsify 文档自动生成脚本
 * 
 * 该脚本可以自动为符合特定结构的文档项目生成 README.md、_sidebar.md 和 _navbar.md 文件
 * 
 * 使用方法:
 *   node generate-docs-final.js [根目录路径] [指定路径名称] [操作类型]
 * 
 * 参数说明:
 *   根目录路径 (可选): 要处理的根目录路径，默认为当前目录
 *   指定路径名称 (可选): 包含所有子分类的主目录名称，默认为"佛藏"
 *   操作类型 (可选): 要执行的操作类型，可选值为：
 *     - 不传参(默认)：生成 sidebar 和 readme
 *     - all：生成所有三种类型的文件
 *     - del：删除三种类型的文件
 *     - nav：单独生成 navbar 文件
 * 
 * 使用示例:
 *   node generate-docs-final.js                    # 生成 sidebar 和 readme
 *   node generate-docs-final.js ./docs docs        # 生成 sidebar 和 readme
 *   node generate-docs-final.js ./docs docs all    # 生成所有三种类型的文件
 *   node generate-docs-final.js ./docs docs del    # 删除三种类型的文件
 *   node generate-docs-final.js ./docs docs nav    # 单独生成 navbar 文件
 */
...
const config = {
  ...
  encoding: 'utf8',
  // 操作类型（可以通过命令行参数传入）
  // 默认生成 sidebar 和 readme，all 表示生成所有文件，del 表示删除文件，nav 表示只生成 navbar
  operationType: process.argv[4] || 'default'
};
 ...
 * @param {Array} fileTypes 要删除的文件类型数组
 */
function deleteFilesRecursively(dir, fileTypes) {
  try {
    // 检查目录是否存在
    ...
    list.forEach(file => {
      ...
      if (stat && stat.isDirectory()) {
        ...
        // 递归处理子目录
        deleteFilesRecursively(fullPath, fileTypes);
      } else {
        // 检查文件是否是要删除的类型
    ...
    console.error(`Error deleting files in ${dir}: ${err.message}`);
  }
}
...
function generateRootNavbarContent(structure) {
  ...
  if (specifiedPathDir) {
    ...
    subDirectories.forEach(subDir => {
      ...
      subDirs.forEach(dir => {
        content += `  - [${dir.name}](${config.specifiedPath}/${subDir.name}/${dir.name}/README.md)\n`;
      });
    });
  }
...
function generateDocs(structure, basePath, isRoot = false) {
  ...
  if (operationType === 'all' || operationType === 'navbar' || operationType === 'nav') {
    // 只在根目录生成导航栏文件
    if (isRoot) {
      console.log(`Generating navbar for: ${structure.path}`);
      const navbarContent = generateRootNavbarContent(structure);
      const navbarPath = path.resolve(structure.path, '_navbar.md');
      fs.writeFileSync(navbarPath, navbarContent, config.encoding);
      console.log(`Created: ${navbarPath}`);
    }
  } else if (operationType === 'navbar' && isRoot) {
    // 兼容旧的 navbar 参数
    console.log(`Generating navbar for: ${structure.path}`);
    const navbarContent = generateRootNavbarContent(structure);
    const navbarPath = path.resolve(structure.path, '_navbar.md');
...
function main() {
  ...
  if (operationType === 'del') {
    ...
    deleteFilesRecursively(targetDir, ['README.md', '_sidebar.md', '_navbar.md']);
  ...
  if (operationType === 'nav') {
    ...
    const navbarContent = generateRootNavbarContent(structure);
    const navbarPath = path.resolve(targetDir, '_navbar.md');
    fs.writeFileSync(navbarPath, navbarContent, config.encoding);
    console.log(`Created: ${navbarPath}`);
